...
with tabs[1]:
    ...
    c1.plotly_chart(model_quality_gauge("Raw map clarity", max(0.05, 1.0 - resolution/6.5)), use_container_width=True)
    c2.plotly_chart(model_quality_gauge("Enhanced clarity", min(0.96, 1.18 - resolution/6.5)), use_container_width=True)
    c3.plotly_chart(model_quality_gauge("Over-sharpening risk", min(0.95, max(0.05, (resolution-2.2)/4.2))), use_container_width=True)
    ...
    st.dataframe(enhance_steps, use_container_width=True, hide_index=True)
...
with tabs[2]:
    ...
    with c2:
        ...
        st.plotly_chart(residue_radar(row), use_container_width=True)
...
with tabs[5]:
    ...
    with c2:
        ...
        """)
        st.dataframe(nt_df[nt_df["review_status"] != "Reliable"].head(18), use_container_width=True, hide_index=True)
...
with tabs[7]:
    ...
    st.dataframe(mutation_df, use_container_width=True, hide_index=True)
    fig = px.scatter(
